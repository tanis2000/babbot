...
namespace Wmo
{
	...
	{
		System.IO.Stream model_stream=null;
	...
	class WDTFile
	{
		...
		public void LoadMapTile(int x, int y) {
			...
			if(wdt.maps[x, y])
			{
				...
				{
                    Console.WriteLine("Reading adt: " + filename);
	...
	{
		MapTile tile;
        LiquidData[] LiquidDataChunk = null; //256 elements
		System.IO.Stream stream=null;
		System.IO.BinaryReader file=null;
		...
		public MapTileFile(string name, MapTile tile, WMOManager wmomanager, ModelManager modelmanager) {
            ...
            //if (ChunkLiquidData != null)
                //Console.WriteLine("ADT HAS MH2O");
            //else
                //Console.WriteLine("ADT HAS MCLQ");
		...
		void HandleChunkMCLQ(MapChunk chunk, uint size) {
			...
			for(int i=0;i<9;i++)
			{
				for(int j=0;j<9;j++)
				{
                    ...
                    //Console.WriteLine("HandleChunkMCLQ: CHUNK.WATIER_HEIGHT[{0}, {1}] = {2}", i, j, chunk.water_height[i, j]);
				}
			}
			...
			for(int i=0;i<8;i++)
			{
				for(int j=0;j<8;j++)
				{
                    ...
                    //Console.WriteLine("HandleChunkMCLQ: CHUNK.WATIER_FLAGS[{0}, {1}] = {2}", i, j, chunk.water_flags[i, j]);
				}
...
0x24 	4 * uint8 	(B,G,R,A) color. Unknown. It is often (0,0,0,255). (something to do with lighting maybe?)
			...
			for(int i=0;i<sets /*wmo.nDoodads*/;i++)
			{
				...
				uint nameOffsetInMODN=file.ReadUInt16(); // 0x00
                file.ReadUInt16(); //throw away 16 bits, these might be flags?. TODO: Confirm this is ok
		...
		private void HandleMODN(uint size) {
            ...
            //TODO: remove
            if (size > 0xFFFF)
                throw new Exception("MODN is larger then 65K, bailing out. Please report this on the forum.");
		}
